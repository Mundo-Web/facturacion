...
const Services = ({ businesses = [], services = [] }) => {
  ...
  const onInviteUser = async (email) => {
    ...
    if (!result) return
    console.log('Marca 1:', servicesByBusiness, serviceLoaded)
              ...
              services.map((service, i) => {
                ...
                return <div key={`service-${i}`} className="card mb-0" style={{ width: '100%', maxWidth: '360px' }}>
                  <div className="card-body project-box">
                    ...
                    <h4 className="mt-0"><a href="" className="text-dark">{service.name} <i className="mdi mdi-arrow-top-right"></i></a></h4>
                            ...
                            <div className="avatar-group">
                              {
                                sbb.users.map((user, i) => (<div key={`user-${i}`} className="avatar-group-item mb-0">
                                  <Tippy content={`${user?.person?.name || user?.name} ${user?.person?.lastname || user?.lastname}`}>
                  ...
                  {serviceLoaded?.users?.map((user, i) => {
                    return <tr key={`user-${i}`}>
                      ...
                      <td align='center'>
                        ...
                        <button className='btn btn-xs btn-white rounded-pill'>
