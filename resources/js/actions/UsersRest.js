...
class UsersRest {
  static paginate = async (params) => {
    const { result } = await Fetch(`/api/users/paginate`, {
      method: 'POST',
      headers: {
  ...
  static status = async ({ id, status }) => {
    try {
      const { status: fetchStatus, result } = await Fetch('/api/users/status', {
        method: 'PATCH',
  ...
  static delete = async (id) => {
    try {
      const { status: fetchStatus, result } = await Fetch(`/api/users/${id}`, {
        method: 'DELETE'
      })
      if (!fetchStatus) throw new Error(result?.message ?? 'Ocurrio un error inesperado')
